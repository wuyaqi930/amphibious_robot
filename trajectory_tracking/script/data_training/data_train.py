...
class neural_network:
    ...
    def __init__(self,data,data_size,BATCH_SIZE): 
        ...
        #生成输入数据
        # self.data_input_numpy = data[:,0:7].astype(np.float)
        # self.data_output_numpy = data[:,7:9].astype(np.float)
        self.data_input_numpy = data[0:200,0:7].astype(np.float)
        self.data_output_numpy = data[0:200,7:9].astype(np.float)
    ...
    def dnn(self):
        ...
        net = Net(n_feature=7, n_hidden=64, n_output=3) 
...
if __name__ == '__main__':
    ...
    #网络训练 
    dnn = neural_network(train_data,45000,20)
