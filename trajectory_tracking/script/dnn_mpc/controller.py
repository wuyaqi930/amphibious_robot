...
#定义一个controller 
#作用 1.接收odom的数据 2.处理ogettdom数据生成控制量u 3. 将控制量u发送出去
class control:
    # 初始化(只初始化一次）
    def __init__(self):
        #时间初始化
    ...
    def mpc_desicion(self,data):
        ...
        #判断此时的位置(使用经验值)
        if -11.5 < x < -12.5 and 9.5 < y < 10.5 : # mpc-target 
            self.is_mpc_line==0
    ...
    def callback(self,data):
        ...
        if self.time % 1000 == 0:
            ...
            self.mpc_desicion(pose_orientation) #将位姿输入，根据经验值来判断

            #3.将data丢给MPC计算
            #3.1 直线版本mpc
            if self.is_mpc_line==1:
                model_predict=mpc_line.MPC(pose_orientation) # 将实时位姿输入 #提示：x_init,x_d还没有 
                model_predict.optimizer()
                print("mpc_line start")

            #3.2 任务版本mpc
            if self.is_mpc_target==1: #需要重新写
    ...
    def data_receive(self):
        ...
        rospy.Subscriber("gazebo/model_states", ModelStates, self.callback)  # topic和消息类型需要和肖岸星商量确定一下 
